...
<div class="py-12">
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Students</h2>

            @can('create', Student::class)
                @unless ($showForm)
                    <x-primary-button wire:click="create">New Student</x-primary-button>
                @endunless
            @endcan
        </div>
        ...
        <div>
            <x-text-input wire:model.live.debounce.300ms="search" placeholder="Search by name or student number&hellip;" class="w-full sm:w-80" />
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Student No.</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Yr</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($students as $student)
                        <tr wire:key="student-{{ $student->id }}">
                            <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $student->student_number }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $student->fullName() }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $student->program->code }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $student->year_level }}</td>
                            <td class="px-6 py-4 text-sm">
                                <span @class([
                                    'px-2 py-1 text-xs rounded-full capitalize',
                                    'bg-green-100 text-green-800' => $student->status === 'active',
                                    'bg-blue-100 text-blue-800' => $student->status === 'graduated',
                                    'bg-gray-100 text-gray-600' => in_array($student->status, ['archived', 'applicant']),
                                    'bg-yellow-100 text-yellow-800' => $student->status === 'transferred',
                                ])>{{ $student->status }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-right space-x-3">
                                <a href="{{ route('enrollment.show', $student) }}" wire:navigate class="text-indigo-600 hover:underline">Enrollment</a>
                                @can('update', $student)
                                    <button wire:click="edit({{ $student->id }})" class="text-indigo-600 hover:underline">Edit</button>
                                @endcan
                            </td>
                        </tr>
                    @empty
