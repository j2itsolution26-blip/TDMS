...
<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @can('viewAny', App\Models\Program::class)
                        <x-nav-link :href="route('programs.index')" :active="request()->routeIs('programs.*') || request()->routeIs('curricula.*')" wire:navigate>
                            {{ __('Programs') }}
                        </x-nav-link>
                    @endcan
                    ...
                    @can('viewAny', App\Models\Student::class)
                        <x-nav-link :href="route('students.index')" :active="request()->routeIs('students.*') || request()->routeIs('enrollment.*')" wire:navigate>
                            {{ __('Students') }}
                        </x-nav-link>
                    @endcan
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('viewAny', App\Models\Program::class)
                <x-responsive-nav-link :href="route('programs.index')" :active="request()->routeIs('programs.*') || request()->routeIs('curricula.*')" wire:navigate>
                    {{ __('Programs') }}
                </x-responsive-nav-link>
            @endcan

            @can('viewAny', App\Models\Subject::class)
                <x-responsive-nav-link :href="route('subjects.index')" :active="request()->routeIs('subjects.*')" wire:navigate>
                    {{ __('Subjects') }}
                </x-responsive-nav-link>
            @endcan

            @can('viewAny', App\Models\Student::class)
                <x-responsive-nav-link :href="route('students.index')" :active="request()->routeIs('students.*') || request()->routeIs('enrollment.*')" wire:navigate>
                    {{ __('Students') }}
                </x-responsive-nav-link>
            @endcan
